...
public class SteamExoSuit extends ArmorItem implements SteamItem, Upgradable {
		...
		if (entity instanceof PlayerEntity) {
			...
			if (slot < 4 && player.getInventory().armor.get(slot) == stack) {
				...
				case 2:
					...
					if ((Integer) getUpgradeFromStack(stack)[5] > 0) {
						player.addStatusEffect(
										new StatusEffectInstance(MRegister.EXOSUIT_KNOCK, 3, 1, false, false));
					}
					break;
	...
	public CompoundTag getUpgradeTag(ItemStack stack, Item... upgrades) {
		...
		for (int i = 0; i < upgrades.length; i++) {
			...
			knockBack = upgrades[i] == Blocks.PISTON.asItem() ? 1 : knockBack;
		}
	...
	public Object[] getUpgradeFromStack(ItemStack stack) {
		...
		return new Object[] {tag.getInt("protecc"), tag.getInt("steamExtra"), tag.getInt("steamJump"),
						tag.getInt("nightVision"), tag.getInt("areaVision"), tag.getInt("knockBack")};
	}
	...
	public Ingredient validUpgrades(Item item) {
		...
		} else if (item == MRegister.STEAM_EXOSUIT_CHEST) {
			items = new Item[] {Items.DIAMOND_CHESTPLATE, MRegister.STEAM_CANISTER, Blocks.PISTON.asItem()};
		} else {
	...
	public Item[] getItemsFromStack(ItemStack stack) {
		...
		if (tag.getInt("knockBack") > 0) {
			items[i++] = Blocks.PISTON.asItem();
		}
		return items;
	}
}
